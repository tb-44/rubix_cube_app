...
export default (sideColors, sliceNumber, forward = true) => {
  ...
  sliceDef.forEach((operation, i, arr) => {
    ...
    const prevTransformType = !prevTransform
      ? null
      : prevTransform[1] !== null
      ? "row"
      : "column";
    ...
    if (!isColumnTransform) {
      ...
      const newRow =
        prevTransformType === "column" && shouldReverse
          ? [...transform].reverse()
          : transform;
      ...
      const getIdx = (idx) =>
        prevTransformType === "row" && sideName !== "right"
          ? Math.abs(idx - 2)
          : idx;
      newSideColors[sideName].forEach((r, idx) =>
        r.splice(column, 1, transform[getIdx(idx)])
      );
    }
  });
  ...
  if (sliceEdges[sliceNumber]) {
    ...
    const rotateReverse = sliceEdges[sliceNumber].includes("!")
      ? forward
      : !forward;
