...
export default (cubes, { slice = null, direction }, spinnerProps) => {
  ...
  const unslicedCubes = [cubes.filter(({ key }) => !sliceKeys.includes(key))];
  ...
  if (adjacentInnerSlices[slice]) {
    ...
    const visibleInnerSlice = allUnsliced.filter(({ key }) =>
      innerSlice.includes(key)
    );
    const restOfUnsliced = allUnsliced.filter(
      ({ key }) => !innerSlice.includes(key)
    );
    unslicedCubes.push(
  ...
  const axis = slice < 3 ? "z" : slice < 6 ? "x" : "y";
